...
const PlantProcessingStatus = () => {
    ...
    const columns = [
        { 
            ...
            render:(text,record) =>(
                <div>{ <span style={{ paddingLeft: '20px', fontsize:'85px'}} >{text}</span> }</div>
            )
        },
        ...
        })),
        {
            ...
            render:(text,record) =>(
                <div>{ <span style={{ paddingLeft: '20px', fontsize:'px'}} >{text}</span> }</div>
            )
        }
    ];
    ...
    return (
        ...
        <Table showHeader={true} columns={columns} dataSource={data} pagination={false} size='large' className='status-table' bordered scroll={{ x: 970, }} />
        // </Card>
    )
