                ...
                <Col span={24}>
                    <Row gutter={[10, 10]}>
                        <Col span={8} xs={24} lg={8}>
                            <Orderdetail buyer={OrderInfo.Buyer} laycan={OrderInfo.laycan}
                                product={OrderInfo.Product} vessel={OrderInfo.vessel}
                                quatity={OrderInfo.quatity}
                            />
                        </Col>
...
const PurchaseOrderProgress = () => {
  ...
  useEffect(() => {
    const request_orderinfo_event = async () => {
      try {
        const response = await Request_order_log(["2023-08-30T16:00:00Z", "2023-12-06T00:00:00Z"]);
        const processedData = processLogData(response);
        setOrderArray(processedData);
      } catch (error) {
