...
const filter_unfinish_order = (Oridata) =>{
    ...
    const data = Oridata.data
    // console.log(data);
    data.map((element) => {
        const FRN = element.fields.parameter
        // console.log(element.fields.parameter);
        if (FRN ===  "Attainment Rate"){
            if(element.fields.value<1){
                filtered_asset.push(element.fields.asset)
                unfinish_order.push(element.fields)
...
const PurchaseOrderProgress = () => {
    ...
    const request_orderinfo_event = async () => {
        ...
        const Filtered_assets = filter_unfinish_order(response_progress_info)

        const response_order_info = await Request_order_log(["2023-01-30T16:00:00Z", "2024-12-06T00:00:00Z"], Filtered_assets)
        console.log(response_order_info);

        const response_order_quantity = await Request_data_log(["2023-08-30T16:00:00Z", "2023-12-06T00:00:00Z"], ["Todate Quantity", "Today Quantity", "Today Targeted Quantity"], Filtered_assets)
