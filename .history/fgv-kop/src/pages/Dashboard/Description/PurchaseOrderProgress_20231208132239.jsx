...
import { BarGraphDataProcess, Individual_Product, LineGraphDataProcess, MergeOrderandProduct, MergeProductProgress, MergeProducttoAsset, Merge_attainment_to_details, Merge_quatity, OrderInfoRawDataProcess, Sort_log_with_asset, getSixMonthsRange, merge_log_and_order } from '../../../DataRequest/LogdataProcess';
...
const PurchaseOrderProgress = () => {
    ...
    const request_orderinfo_event = async () => {
        ...
        // const response_product_log = await Request_order_log(daterange, [])

        OrderArray = merge_log_and_order(Merge_quatity(Merge_attainment_to_details(OrderInfoRawDataProcess(response_order_info), response_progress_info), response_order_quantity), response_order_quantity)
        ...
        const response_product_log = await Request_data_log(daterange, ["Todate Quantity (RBDPO)", "Todate Quantity (RBDPL)",
            "Todate Quantity (RBDPS)", "Todate Quantity (PFAD)"], AssetName)
        const asset_product = MergeProducttoAsset(response_product_log)

        // console.log(response_product_log);
        ...
        MergeOrderandProduct(OrderArray,OrderProducts)
            ...
            <Col span={24}>
                ...
                <Space direction="vertical" size="small" style={{ display: 'flex', color: 'white' }}>
                    ...
                    <div style={{ paddingTop: '5px', fontWeight: 'bold', padding: ' 0px 0px 0px 0px' }}>
                        ...
                        <Progress percent={progress.produced / progress.produced} strokeColor={twoColors} size={'small'} strokeWidth={8} format={customFormat} />
                    </div>
