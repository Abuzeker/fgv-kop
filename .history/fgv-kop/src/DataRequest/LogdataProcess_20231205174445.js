...
export function Individual_Product(rawdata) {
    ...
    // Extract unique products from rawdata
    ...
    uniqueProducts.forEach(product => {
        ...
        rawdata.forEach(order => {
            // produced += parseInt(order[`Quantity (${product})`]) || 0;
            // remaining += parseInt(order.Quantity) - produced;
            todayTarget += order['Daily Quantity'].reduce((acc, daily) => acc + (daily['Today Targeted Quantity'] || 0), 0);
        });
