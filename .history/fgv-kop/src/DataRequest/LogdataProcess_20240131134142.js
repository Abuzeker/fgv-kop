...
export function TenDaysbeforesRange() {
    ...
    // Calculate 10 days before
    const tenDaysBefore = new Date(currentDate);
    tenDaysBefore.setDate(currentDate.getDate() - 15);
...
export const BarGraphDataProcess = (data) => {
    ...
    data.map((element) => {
        ...
        dailydata.map((element2) => {
            ...
            element2["Efficiency"] = isNaN(element2["Efficiency"]) ? 0 : parseFloat(element2["Efficiency"]).toFixed(2);
            element2["Efficiency"] = !isFinite(element2["Efficiency"]) ? 100 :  parseFloat(element2["Efficiency"]).toFixed(2);
            Graph_data.push(element2)
        })
    })
...
export const Merge_parameter_name_and_value_line = (response, assets) => {

    const Line_value_name = []
    const obj={}


    assets.forEach(element => {

        const Filteredasset =  response.data.filter(item => item.fields.asset === element);
        // console.log(Filteredasset);

        const transformedData = Filteredasset.reduce((acc, item) => {
            const { parameter, value } = item.fields;
            acc[parameter] = value;
            return acc;
        }, {});
        ...
        obj[element] = transformedData
        // Line_value_name.push(obj[element])
    });
    // console.log(obj);
...
export const Merge_parameter_name_and_value_line_obj = (response, assets) => {
