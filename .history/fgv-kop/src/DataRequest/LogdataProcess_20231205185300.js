export function MergeProductProgress(data1, data2) {
    const mergedData = [];
    ...
    data1.forEach(item1 => {
        const matchingItem2 = data2.find(item2 => item2.Asset === item1.Asset);

        if (matchingItem2) {
            // Merge items with the same Asset
...
export function MergeProducttoAsset(rawdata) {
    ...
    data.forEach(product => {
        ...
        obj[product.fields.parameter] = product.fields.value;
