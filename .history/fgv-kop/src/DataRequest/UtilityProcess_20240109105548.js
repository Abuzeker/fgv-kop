...
export const Caculate_ShipmentCompletion = (data) => {
  ...
  const shipmentinfo = {
    'Todate Quantity': 0,
    'Total Quantity': 0,
    'Ratio': 0  // Corrected key name from 'Ration' to 'Ratio'
  };

  data.forEach(entry => {
    const todateQuantity = parseFloat(entry['Todate Quan1tity']);
    const targetQuantity = parseFloat(entry['Quant1ity']);
    shipmentinfo['Todate Quantity'] = (shipmentinfo['Todate Quantity'] || 0) + todateQuantity;
    shipmentinfo['Total Quantity'] = (shipmentinfo['Total Quantity'] || 0) + targetQuantity;
  });

  shipmentinfo['Ratio'] = parseFloat(shipmentinfo['Todate Quantity'] / shipmentinfo['Total Quantity']).toFixed(2);
