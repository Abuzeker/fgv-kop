...
export function findHighestTodateCPO(data) {
  ...
  data.forEach(obj => {
    ...
    try {
      const tolerance = 0.001; // Set a small tolerance value

      todateRBDPOValue = parseFloat(obj["Todate (RBDPO)"]).toFixed(1);
      console.log(todateRBDPOValue);
      console.log(highestValueRBDPO);
      
      if (todateRBDPOValue > highestValueRBDPO + tolerance) {
          console.log('bigger');
      } else if (todateRBDPOValue < highestValueRBDPO - tolerance) {
          console.log('smaller');
      } else {
          console.log('equal or approximately equal');
      }
    } catch (error) {
      todateRBDPOValue = 0
    }
